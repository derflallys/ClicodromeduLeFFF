...
export class CombinaisonComponent implements OnInit {
  ...
  onSelectCategory($id) {
    ...
    this.service.getCombinaison(this.categorySelected).subscribe(
        combin => {
          console.log(combin);
  ...
  onSubmit() {
    ...
    this.service.addCombinaison(tagCategory).subscribe(response => {console.log(response) ; if (response.status === 200) {
